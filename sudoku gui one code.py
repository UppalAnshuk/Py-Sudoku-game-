...
class SudokuGUI(Frame):
    ...
    def make_grid(self):
        c = Canvas(self, bg='blue', width='600', height='600')
        ...
        for y in range(9):
            for x in range(9):
                ...
                t = c.create_text(xr+rsize/2, yr+rsize/2, text="SUDOKU",
                                  font="System 15 bold")
    ...
    def canvas_key(self, event):
        if event.char.isdigit() and int(event.char) > 0 and self.current:
            ...
            try:
                self.canvas.itemconfig(self.handles[y][x][0], fill='blue')
                self.board.set(x, y, int(event.char))
                self.sync_board_and_canvas()
            except ValueError:
                self.canvas.itemconfig(self.handles[y][x][0], fill='red')
    def __init__(self, master, board):
        Frame.__init__(self, master)

        if master:
            master.title("SUDOKU v1.00")
        ...
        label1=Label(bframe,bg='white',fg='white')
        label1.pack(side="left",fill="x",expand='1')
        label1.config(text=Clock.tick())
